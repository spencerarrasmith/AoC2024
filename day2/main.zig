...
pub fn part2() !void {
    ...
    for (data.items) |report| {
        ...
        //    std.debug.print("{d} ", .{level});
        //}
        const reversed = try report.clone();
        std.mem.reverse(i32, reversed.items);
        defer reversed.deinit();
        const inc: bool = try verify_increasing_dampener(report);
        const inc2: bool = try verify_increasing_dampener(reversed);
        const dec: bool = try verify_decreasing_dampener(report);
        const dec2: bool = try verify_decreasing_dampener(reversed);
        //std.debug.print("{any} {any} {any} {any}\n", .{ inc, inc2, dec, dec2 });
        if (inc or dec or inc2 or dec2) {
            count_safe += 1;
        }
    }
    std.debug.print("{any}\n", .{count_safe});
...
pub fn verify_increasing_dampener(report: std.ArrayList(i32)) !bool {
    var current_level: i32 = report.items[0];
    var bad: u8 = 0;
    for (report.items[1..]) |level| {
        if ((level > current_level) and ((level - current_level) <= 3)) {
...
pub fn verify_decreasing_dampener(report: std.ArrayList(i32)) !bool {
    var current_level: i32 = report.items[0];
    var bad: u8 = 0;
    for (report.items[1..]) |level| {
        if ((level < current_level) and ((current_level - level) <= 3)) {
